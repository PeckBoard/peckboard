...
const TERMINAL_TOOL_DISABLED_MSG: &str = "The terminal/Bash tool is disabled in Peckboard. Use the `run_command` tool instead to run shell commands.";
...
pub(super) fn check_path_violation(
    ...
    if is_terminal_tool(tool_name) {
        return Some(TERMINAL_TOOL_DISABLED_MSG.to_string());
    }
...
mod tests {
    ...
    fn bash_is_denied_with_run_command_message() {
        for tool in ["Bash", "BashOutput", "KillShell"] {
            ...
            assert_eq!(
                denied.as_deref(),
                Some(TERMINAL_TOOL_DISABLED_MSG),
    ...
    fn bash_denied_even_when_allowed_dir_empty() {
        ...
        assert_eq!(denied.as_deref(), Some(TERMINAL_TOOL_DISABLED_MSG));
    }
