...
Core screenshots (`assets/screenshots/*.png`) are generated by
`web/e2e/screenshots/screenshots.spec.ts` (`cd web && npm run
screenshots`), which boots its own server and stubs plugin surfaces.
Plugin-page screenshots (`assets/screenshots/plugins/*.png`) come from
`web/scripts/docs-plugin-shots.mjs`, which captures the REAL plugin
pages: boot a scratch instance with a throwaway data dir, install and
approve every registry plugin, seed a folder with a git repo, a
project with cards, mock sessions, SSH hosts, and an orchestrator,
then run the script with `PB_URL`/`PB_USER`/`PB_PASS` pointing at it
(`ONLY=name1,name2` re-captures a subset).
```
