...
[Experts]({{ "/experts.html" | relative_url }}) covers the long-lived sessions that workers consult before acting, and [Plugins]({{ "/plugins.html" | relative_url }}) the installable modules that add features like the diff viewer and SSH tools. [Architecture]({{ "/architecture.html" | relative_url }}) describes the server, database, and MCP layer that hold everything together, and [Configuration]({{ "/configuration.html" | relative_url }}) lists the command-line flags and where data lives.
