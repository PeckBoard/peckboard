...
Experts are ordinary sessions in the same SQLite database as everything else, flagged with an expert kind, so they survive server restarts.
...
The question and PM experts are permanent sessions with stable, deterministic ids derived from the project, so they rehydrate under the same id after a restart and a repeated spin-up never clobbers what they have accumulated.
