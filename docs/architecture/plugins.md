...
hook and the `register_provider` permission, or it fails to load. The
aspirational hooks from earlier drafts — `provider.register.before/after/failed`,
`provider.spawn.*`, `provider.send.before/after`, `provider.event`,
`provider.interrupt.*`, `provider.kill.*`, `provider.cleanup.after`,
`provider.models.list` — are **not implemented**; interrupts are delivered via
the `peckboard_provider_should_stop` host-function poll instead of a hook.
