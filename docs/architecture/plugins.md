...
| peckboard_create_session          | Create a new session                                                                                                                                                                                                                                                                                                          | Not yet implemented |
| peckboard_append_event            | Append an event to a session's log                                                                                                                                                                                                                                                                                            | Not yet implemented |
...
> Note: there is currently no per-plugin gate on host functions (unlike hook
> registration, which is gated by `ALLOWED_HOOKS`). Every loaded `.wasm` plugin
> can call all implemented host functions, including the `peckboard_create_card`
> write. Anything dropped into `<dataDir>/plugins/` is trusted to run in-process.
