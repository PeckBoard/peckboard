...
| timer.tick             | Every ~30s, on the repeating-task scheduler's cadence              | now (RFC 3339)                                        | No         | No                             |
...
> `timer.tick` is a **notification** dispatched via `dispatch_notify` from the repeating-task scheduler loop in `src/server.rs` — the plugin-side scheduler primitive. A plugin that declares it gets woken every ~30s to check its own due timers/watchdogs; core knows nothing about them. Payload: `{ now }`.
...
A plugin can register an **AI provider** (see `docs/architecture/providers.md`,
"Plugin Providers"). Two provider hooks are implemented; they are dispatched
per declaring plugin, not broadcast:

| Hook               | When                                                                                                                                         | Payload                                                                                  | Can cancel           | Can modify                                             |
| ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------ |
| provider.register  | At startup after plugins load, and whenever the plugin set changes (approve/deny, install, uninstall)                                        | `{}` — the plugin answers by calling `peckboard_register_provider`                       | No                   | No                                                     |
| provider.send      | One agent turn on the plugin's provider (runs on a dedicated blocking thread with the provider-send budget)                                  | `{session_id, provider_id, spawn_config, message: {text, attachments}, conversation_id}` | Yes (fails the turn) | No                                                     |
| provider.models    | A model-catalog read (`/api/models`, MCP `list_models`) — optional; dispatched with a `try_lock` so it never stalls behind a turn            | `{provider_id}`                                                                          | No                   | Yes (`Allow {models}` replaces the registered catalog) |
| provider.interrupt | The user interrupted a turn on the plugin's provider — optional; queues behind the in-flight `provider.send` on the plugin's single instance | `{session_id, provider_id}`                                                              | No                   | No (verdict ignored)                                   |

A plugin declaring `provider.register` must also declare the `provider.send`
hook and the `register_provider` permission, or it fails to load; likewise
`provider.models` and `provider.interrupt` each require `provider.register`.
The aspirational hooks from earlier drafts — `provider.register.before/after/failed`,
`provider.spawn.*`, `provider.send.before/after`, `provider.event`,
`provider.kill.*`, `provider.cleanup.after` — are **not implemented**.
Interrupts still work through the `peckboard_provider_should_stop` poll:
`provider.interrupt` is a cleanup notification on top of that flag, never a
preemption of the running turn.
...
**Theme handshake.** The host forwards the user's appearance choice to every embedded plugin page (dropdown panels and full-page items alike): when the stored Peckboard theme is explicitly `light` or `dark`, `withPluginTheme` (`web/src/util/appearance.ts`) appends `?theme=light|dark` to the iframe `src`; in auto mode no param is sent. The plugin page stamps the param on its own root (`<html data-theme="…">`, the stamp winning over the OS preference) and otherwise follows `prefers-color-scheme` — the same resolution core's auto mode uses, so host and iframe always agree. Plugin CSS mirrors core's three-block token structure (`:root` light, `:root[data-theme="dark"]`, and a `prefers-color-scheme: dark` block scoped to `:root:not([data-theme="light"])`); `peck-plugins/graphify/page/style.js` is the reference implementation.
...
| peckboard_list_models                                                          | The selectable model catalog for session creation, across every non-hidden provider — **thinking models only**, filtered server-side (a non-thinking model is absent, not flagged). Reads the same `ProviderRegistry` as `GET /api/models`, but through the dynamic∪static union (`list_providers_with_models_union_except`) so a registry-known model a stale external catalog hides (e.g. `claude-fable-5` behind an older `claude` CLI) stays listed. Request: `{}`. Response: `{"models": [{"id", "display_name", "provider", "account_id", "thinking", "tier"}]}` — ids are account-qualified (`claude:claude-sonnet-4-6@acc_xxx`; split on the LAST `@`), `account_id` null for the default account. **Metadata only — never credentials, tokens, or account material.** Gated by **`models_read`**.                                                                                                                                                                                                                                                                                                                                                                                | ✅ Implemented      |
...
| peckboard_caller_scope                                                         | The folder/project/session this call is running in, as core already resolved it: the in-flight MCP invocation's verified scope, or the scope of the authenticated page request (`resolve_authed_scope`). Request: `{}`. Response: `{"folder_id", "project_id", "session_id", "authority"}` — all null outside either (e.g. `init`). **Ungated**: it reports only where the plugin already is. Needed because the document store is scoped by _plugin_, not folder, so per-folder state has to carry the folder in its key — and on the authed UI surface there is no other way to name it (`peckboard_get_session` is invocation-only and ownership-gated). `authority` distinguishes an operator's click from an agent's tool call.                                                                                                                                                                                                                                                                                                                                                                                                                                                      | ✅ Implemented      |
...
| peckboard_create_session                                                       | Create a generic session in the **caller's** folder + project (taken from the trusted caller context, never plugin-supplied). `{name, id?, model?, effort?, is_expert?, expert_kind?, is_temp?, folder_path?, folder_name?}` → `{session}`. `is_expert`/`expert_kind` (default false/None) classify the session for usage attribution and listings; expert _knowledge_ state is still the plugin's own `session_meta`. `is_temp` (default false) marks a temp session the UI deletes when its tab closes. `folder_path` (authority-only — refused for MCP tool invocations, which stay pinned to their caller's folder) lands the session in the folder registered at that path, find-or-registering it server-side like the core install-folder flow: leading `~/` expands to the server user's home, same unsafe-path refusal as `POST /api/folders`, directory created, exact-path row reused; `folder_name` names a newly registered folder. Callable from a tool invocation **or** an authenticated plugin-UI request under `user_authority` (the `TrustedCaller` proof token in `src/plugin/host.rs`); refused with neither. Gated by **`session_write`**.                          | ✅ Implemented      |
...
| peckboard_set_session_system_prompt                                            | Set (or clear) the standing instructions a session runs under — the plugin twin of core's `set_session_system_prompt` MCP tool. Request: `{"session_id", "system_prompt"?, "name"?}`. An explicit `system_prompt` body wins and clears the recorded library name (a raw body has no name); a `name` resolves through the prompt library, writing its body **and** recording the reference; neither clears both, reverting the session to the standing Peckboard prompt. Bodies over 100 000 chars are rejected. Response: `{"session_id", "session_name", "system_prompt_set"}`. Gated by **`session_prompt_write`**; authorized by **visibility only** (not ownership) — a plugin may steer a session it did not create, inside the caller's folder/project.                                                                                                                                                                                                                                                                                                                                                                                                                             | ✅ Implemented      |
...
| peckboard_session_events                                                       | Read a **slim tail** of a session's event log for visualization. Request: `{"session_id", "after_seq"?, "limit"?}`. Response: `{"events": [{"seq", "kind", "name"}], "latest_seq"}` — `name` is the event's tool name when its payload carries one, else null; `latest_seq` is the last returned event's seq, or null when none. **Slim: no event `data` payloads** (activity metadata only). Gated by **`session_read`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | ✅ Implemented      |
...
| peckboard_dispatch_capture / resume_session                                    | **Fire-and-forget** agent dispatch on a session in the caller's scope (capture run, or deliver + resume — e.g. hand an expert a question, or an answer back to the asker). `{session_id, prompt, clear_first?}` / `{session_id, text}` → `{ok:true}`. `clear_first` wipes the session (the `peckboard_clear_session` wipe) before dispatching, ordered inside one host-side task — separate clear + dispatch calls are unordered spawns and the wipe can kill the fresh run; it additionally requires **`session_control`** (the project-planner uses it to reset its interview session's context after every answered question). Gated by **`session_dispatch`**; authorized by visibility. Callable from a tool invocation **or** an authenticated plugin-UI request under `user_authority` (`dispatch_capture` carries the `TrustedCaller` proof token). Needs the live host bound (set after startup); inert in headless managers.                                                                                                                                                                                                                                                    | ✅ Implemented      |
| peckboard_orchestrate_send / \_create_session / \_set_prompt / \_session_state | **Unattended session control** — the context-free quartet for orchestrating plugins, callable from lifecycle dispatches (`timer.tick`, `session.agent.ended`) where every context-checked twin refuses. `orchestrate_send`: `{session_id, text}` → deliver + resume (fire-and-forget; needs the live host). `orchestrate_create_session`: `{folder_id, name, model?, effort?, system_prompt?}` → `{session}` — the folder is named explicitly (no caller scope to inherit) and must exist. `orchestrate_set_prompt`: `{session_id, system_prompt?}` → set/clear standing instructions (100k cap). `orchestrate_session_state`: `{session_id}` → `{exists, session?: {id, name, folder_id, model, is_worker, last_activity}}`. Folder-blind, no per-call approval — the **`session_orchestrate`** permission is a standing grant the operator approves once at plugin approval.                                                                                                                                                                                                                                                                                                            | ✅ Implemented      |
| peckboard_http_fetch                                                           | Outbound **public-web** HTTP on the plugin's behalf. Request: `{"url", "method"?: "GET"\|"HEAD", "headers"?: {..}}`. Response: `{"status", "headers", "body", "truncated", "final_url"}`. SSRF-contained: http/https only, GET/HEAD only, the resolved IP is checked against private/loopback/link-local/CGNAT ranges and pinned (no DNS rebinding), redirects returned verbatim, 5 MiB / 20 s caps. Gated by **`http_fetch`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | ✅ Implemented      |
| peckboard_http_request                                                         | Outbound HTTP with **any standard method and private/LAN targets allowed** — for plugins that integrate self-hosted services (e.g. an nginx-proxy-manager MCP endpoint on the LAN). Request: `{"url", "method"?: "GET"\|"HEAD"\|"POST"\|"PUT"\|"PATCH"\|"DELETE", "headers"?: {..}, "body"?, "timeout_secs"?: 1..=120 (default 30)}`. Response as `peckboard_http_fetch`. Server-side request forgery **by design** (it can reach the internal network, including Peckboard's own API), hence its own operator-approved **`http_request`** permission; still http/https only, pinned resolution, no redirects, 5 MiB cap.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | ✅ Implemented      |
| peckboard_ssh_key_list                                                         | List **vault key metadata** — id, name, key type, fingerprint, whether a passphrase is set, created-at. Never the private key, its ciphertext/nonce, or the passphrase. Request: `{}`. Response: `{"keys": [{"id", "name", "key_type", "fingerprint", "has_passphrase", "created_at"}]}`. Gated by the **`ssh_keys`** permission.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | ✅ Implemented      |
| peckboard_ssh_probe / exec / read_file / write_file                            | Native **SSH** to a remote host via russh — credentials (password, private key, or a vault key reference) are passed inline and held in memory only, **never written to disk**. Shared connection fields: `{host, port?(=22), username, auth: {password} \| {private_key, passphrase?} \| {key_id}, known_host?, connect_timeout_secs?}`. The `{key_id}` form resolves a key stored in the SSH key vault (see "SSH Key Vault" below) by id and additionally requires the **`ssh_keys`** permission — a plugin with only `ssh` gets `{"error": "plugin lacks the 'ssh_keys' permission required to use a stored SSH key"}`. `probe`: connect + authenticate → `{ok, server_fingerprint, latency_ms}` (trust-on-first-use — pass the returned fingerprint back as `known_host` to pin it; a mismatch aborts the handshake). `exec`: `+ {command, timeout_secs?}` → `{exit_code, stdout, stderr, stdout/stderr_truncated, timed_out, duration_ms}`, 1 MiB/stream. `read_file` / `write_file`: SFTP with a base64 body, 1 MiB cap. Connections are pooled per `(host, port, user, auth)` with an idle TTL. Gated by the **`ssh`** permission (plus `ssh_keys` for `{key_id}` auth, as above). | ✅ Implemented      |
| peckboard_update_card                                                          | Update fields on an existing card. Request: `{"card_id": "<id>", "title"?, "description"?, "step"?, "priority"?: 0-3, "model"?, "effort"?: low\|medium\|high\|xhigh\|max, "blocked"?: bool, "block_reason"?"}` — all fields except `card_id` are optional; omitted fields are unchanged. Response: `{"card": <Card>}` or `{"error": "..."}`. Gated by **`cards_write`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ✅ Implemented      |
...
| peckboard_register_provider                                                    | Register an AI provider on the calling plugin's behalf. Request: `{"id": "[a-z0-9_-]", "display_name", "models": [{id, display_name, capabilities?, tier?}], "effort_levels"?: [{id, label}], "pricing"?: {model_id: {input_usd_per_mtok, output_usd_per_mtok}}}`. Response: `{"ok": true}`. Only meaningful during a `provider.register` dispatch — core stages the registration and applies it (collision-checked: a plugin can never displace a provider it doesn't own) when the dispatch returns. Gated by **`register_provider`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | ✅ Implemented      |
| peckboard_emit_provider_event                                                  | Stream one `ProviderEvent` into the session whose `provider.send` turn this plugin is currently executing. Request: `{"session_id", "event": {"kind": "started"\|"text"\|"tool_start"\|"tool_end"\|"todo"\|"usage"\|"completed"\|"crashed"\|"control_request", ...}}`. Response: `{"ok": true}`. Feeds the shared `emit_event` path (DB append, `usage_events`, `conversation_id` persistence, WS broadcast). Guarded: only the plugin running the session's active turn may emit, and nothing may be emitted after `completed`/`crashed`. Gated by **`register_provider`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | ✅ Implemented      |
| peckboard_provider_should_stop                                                 | Cooperative-interrupt poll for the current turn. Request: `{"session_id"}`. Response: `{"stop": bool}` — `true` once the user interrupts/cancels (or when the caller doesn't own an active turn for the session). Poll it between HTTP chunks; the per-call WASM timeout is the hard backstop. Gated by **`register_provider`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | ✅ Implemented      |
| peckboard_provider_take_message                                                | Pop a user message core handed to the live turn mid-flight (mid-stream injection). Request: `{"session_id"}`. Response: `{"message": {"text", "attachments": [{filename, mime_type, data_base64}]}\|null}`. Only ever non-null for a provider registered with `supports_mid_stream_injection: true` — declare that flag ONLY if the turn actually polls this, since core then skips the durable `queued_messages` path. Gated by **`register_provider`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | ✅ Implemented      |
| peckboard_provider_get_session                                                 | Trusted session context for the turn being executed. Request: `{"session_id"}`. Response: `{"session_id", "folder_path", "card_id"?, "project_id"?, "is_worker"}` — served from host-side dispatch state, never plugin-supplied ids. Gated by **`register_provider`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | ✅ Implemented      |
| peckboard_provider_get_mcp_config                                              | Path of the per-session MCP config (`worker-mcp/<session_id>.json`) for the turn being executed, as resolved on its `SpawnConfig`. Request: `{"session_id"}`. Response: `{"path": string\|null}`. Gated by **`register_provider`**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | ✅ Implemented      |
...
> `session_write` / `session_prompt_write` / `event_append` /
> `project_files_read` / `session_dispatch` / `cards_write` /
> `register_provider` / `ssh` / `ssh_keys` / `models_read` /
> `session_orchestrate`),
...
> The **`ssh_keys`** permission is narrower than it looks: it grants **listing
> vault key metadata** (`peckboard_ssh_key_list` — id, name, key type,
> fingerprint, whether a passphrase is set) and **using a vault key by id**
> (`{key_id}` as the `auth` field of `peckboard_ssh_probe` / `_exec` /
> `_read_file` / `_write_file`). It does **not** grant reading private key
> material — the private key, its ciphertext/nonce, or the passphrase never
> cross the host-function boundary into a plugin. See "SSH Key Vault" below.
> The **`models_read`** permission likewise exposes **metadata only**: model
> ids, display names, provider ids, account ids, and tiers — never API keys,
> OAuth tokens, or any other account credential material.
>
...
> own permission. `peckboard_create_session` and `peckboard_dispatch_capture`
> make the two valid contexts explicit via the `TrustedCaller` proof token
> (`src/plugin/host.rs`): they succeed from a tool invocation or an authed
> request under `user_authority`, and refuse with neither.
> (`provide_mcp_tools`, `contribute_sidebar`, and `broadcast`
...
material itself. Storage is the `ssh_keys` table (`db::crud::ssh_keys`,
`src/db/models.rs`'s `SshKey`) — id, name, key type, fingerprint, and the
private key (plus optional passphrase) as separate ciphertext/nonce column
pairs. Rows are written by `POST /api/ssh-keys` (import) and
`POST /api/ssh-keys/generate` (`src/routes/ssh_keys.rs`, admin-only — a key has
no `user_id` to scope it to, same reasoning as `routes/env_vars.rs`), never by
a plugin.

Encryption (`src/service/ssh_keys.rs`) is AES-256-GCM under a **server-held
vault key**: a random 32-byte key generated on first use and persisted at
`<data_dir>/ssh_vault_key` with `0600` permissions (Unix), mirroring
`auth::token::load_or_create_jwt_secret`'s on-disk lifecycle. This is a
deliberately different scheme from `service::env_vars`, which derives its key
from the user's login password via Argon2id: SSH keys are used **headlessly**
by agents, workers, and repeating tasks, and a password-bound scheme would
stall that work on an unlock prompt every session.

**Threat model — read this before treating the vault as a secrets manager.**
The vault key sits in the same data directory as the SQLite database it
encrypts, so encryption at rest protects a **database dump or backup** taken
without the data directory's vault-key file, and nothing more:

- **Protects against**: a stolen/leaked `peckboard.db` file, a DB backup
  uploaded somewhere it shouldn't be, casual inspection of the SQLite file.
- **Does not protect against**: a full host compromise. Anyone who can read
  `<data_dir>/ssh_vault_key` can decrypt every stored private key — and that
  file lives right next to the database on the same machine.

Contrast with `env_vars`, which is bound to the user's login password and
therefore survives a host compromise that doesn't also capture that password —
but is unusable by anything headless (a worker, a repeating task) until a
logged-in user unlocks it. Vault keys trade that headless-availability
...
reviewing a manifest — `ssh-fleet` holds no exec permission; `app-manager`
