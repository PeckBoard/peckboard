...
/// [`restart`]. Returns the verified sha256 (hex) on success.
pub async fn download_and_swap(client: &reqwest::Client, tag: &str) -> Result<String> {
    let asset =
...
/// Write `bytes` to a temp file beside the current executable and rename it over
/// the executable. The temp lives in the same directory so the rename is atomic
/// (same filesystem). On Unix, renaming over a *running* executable is safe —
...
/// Re-exec the (already-replaced) current executable with the original args and
