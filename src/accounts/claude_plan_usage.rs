...
/// tag, never a block.
#[allow(dead_code)] // model-picker "not configured" hint; no caller after provider extract
...
pub async fn refresh_once(db: &Db) {
    ...
    for acct in accounts.iter().filter(|a| a.kind == "oauth_token") {
        ...
        let token = match crate::accounts::claude_token_refresh::fresh_credential(db, acct).await {
            Ok(t) => t,
            Err(e) => {
