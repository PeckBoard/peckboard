...
impl SessionManager {
    ...
    pub async fn send_message_locked(
        ...
        let final_config = SpawnConfig {
            ...
            // how the message reached it, so derive the flag here from the
            // session rather than trusting each caller to set it on the
            // incoming config.
            restrict_to_qa: session.expert_kind.as_deref() == Some("question"),
        };
