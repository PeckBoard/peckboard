...
mod auto_tests {
    ...
    #[test]
    fn working_style_tells_agents_to_use_spawn_subagent_not_native_tools() {
        assert!(WORKING_STYLE.contains("spawn_subagent"));
        assert!(WORKING_STYLE.contains("built-in subagent/task tool"));
        assert!(WORKING_STYLE.contains("Task/Agent"));
        assert!(WORKING_STYLE.contains("grok's `task`"));
        assert!(WORKING_STYLE.contains("kimi's Task"));
    }
