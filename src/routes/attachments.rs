...
pub fn router(state: Arc<AppState>) -> Router<Arc<AppState>> {
    Router::new()
        ...
        // Layers run outer-to-inner on the request, so `require_session_access`
        // is appended LAST and executes AFTER `require_auth` has put
        // `AuthUser` into the request extensions. Applies the same
        // owner-or-shared-board rule as the sessions routes.
