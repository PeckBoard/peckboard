...
/// Tell every connected client the review moved. The event is keyed by the
/// review id (not a session id) so the review screen can subscribe to it the
/// same way a session subscribes to its own stream.
...
async fn broadcast_by_id(state: &AppState, id: &str) {
    if let Ok(Some(review)) = state.db.get_doc_review(id).await {
