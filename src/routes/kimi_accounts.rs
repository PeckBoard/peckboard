//! `/api/kimi-accounts` — manage the set of Moonshot AI / Kimi Code
//! credentials the spawned `kimi` CLI can run as. Mirrors
//! [`super::grok_accounts`]; see [`super::claude_accounts`] for the
//! budget/usage model. The "Default" account (host `~/.kimi-code`) is
//! implicit and never appears here; a session uses an account by carrying
//! `@<account_id>` on its model id.
//!
//! Kimi's login is a browser device-code flow run by the CLI itself, so —
//! like Grok and unlike Claude's paste-back exchange — an account is created
//! first (in a not-yet-authenticated state) and then signed in via
...
/// tokens live in `config_dir/config.toml`.
...
/// Whether the account is usable: a device account once `kimi login` has
/// written its config.toml, an api_key account once it has a key.
fn is_authenticated(acct: &KimiAccount) -> bool {
...
async fn account_usage(state: &AppState, acct: &KimiAccount) -> anyhow::Result<AccountUsage> {
...
async fn to_view(state: &AppState, acct: KimiAccount) -> anyhow::Result<AccountView> {
...
async fn list_accounts(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let accounts = match state.db.list_kimi_accounts().await {
        Ok(a) => a,
...
async fn create_account(
    ...
    // `kacc_<hex>` — deliberately free of `@` so it never collides with the
    // model-id account separator, and `k`-prefixed to set it apart from a
    // Claude `acc_` / Grok `gacc_` id at a glance.
    let id = format!("kacc_{}", uuid::Uuid::new_v4().simple());
    ...
    // An api_key account is usable immediately: materialise its
    // KIMI_CODE_HOME config (providers + model aliases + key) up front.
    if kind == "api_key"
        && let Err(e) = login::write_api_key_config(&config_dir, &credential)
    {
        return Err(server_error(format!("failed to write account config: {e}")));
    }

    let now = now_ms();
    let new = NewKimiAccount {
        id,
        name,
        kind,
        credential,
    ...
    match state.db.create_kimi_account(new).await {
...
async fn update_account(
    ...
    let changes = KimiAccountChanges {
        name: Some(name),
    ...
    match state.db.update_kimi_account(&id, changes).await {
        Ok(Some(acct)) => {
            // A replaced key re-materialises the account's config.toml so the
            // next turn runs with it.
            if acct.kind == "api_key"
                && credential.is_some()
                && let Some(dir) = acct.config_dir.as_deref()
                && let Err(e) = login::write_api_key_config(dir, &acct.credential)
            {
                return Err(server_error(format!("failed to write account config: {e}")));
            }
...
async fn delete_account(
    ...
    // Stop any in-flight login before removing the row / its KIMI_CODE_HOME.
    KIMI_LOGIN.cancel(&id).await;
    ...
    let result = crate::routes::account_delete_guard::guarded_delete(
        ...
        &id,
        query.force,
        move || async move { db.delete_kimi_account(&del_id).await },
    )
    .await;
    match result {
        Ok(config_dir) => {
...
/// Begin a device login for an existing account: spawn `kimi login` against
/// the account's KIMI_CODE_HOME and return the sign-in URL. The account
/// reads as `authenticated` once the browser sign-in completes.
async fn start_login(
    State(state): State<Arc<AppState>>,
    Path(id): Path<String>,
) -> impl IntoResponse {
    let account = match state.db.get_kimi_account(&id).await {
        Ok(Some(a)) => a,
        Ok(None) => {
            return Err((
                StatusCode::NOT_FOUND,
...
mod tests {
    ...
    fn account(kind: &str, credential: &str, config_dir: Option<&str>) -> KimiAccount {
        KimiAccount {
            id: "kacc_x".into(),
            name: "n".into(),
            kind: kind.into(),
    ...
    fn is_authenticated_keys_off_kind() {
        ...
        assert!(is_authenticated(&account("api_key", "sk-123", None)));
        assert!(!is_authenticated(&account("api_key", "", None)));
        // device: authenticated only when config.toml exists (no dir → false).
