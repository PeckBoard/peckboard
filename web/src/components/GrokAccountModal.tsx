...
import type { GrokAccount, GrokAccountKind } from '../types/api'
...
interface Props {
  ...
  onSaved: (signIn: GrokAccount | null) => void
  onClose: () => void
}
...
const KIND_OPTIONS: { value: GrokAccountKind; label: string; hint: string }[] = [
  {
    value: 'device',
    label: 'Sign in',
    hint: 'Sign in with your Grok account in the browser (device code).',
  },
  {
    value: 'api_key',
    label: 'API key',
    hint: 'Paste an xAI API key (xai-…). Billed via the API.',
  },
]

/**
 * Add or edit a Grok account. Unlike Claude's paste-back token flow, a Grok
 * `device` account is created here first and then signed in via the browser
 * (the row's "Sign in" button / {@link GrokSignInModal}) — so this modal only
 * collects the name, kind, optional API key, and budget. On creating a new
 * device account it hands the account back so the caller can launch sign-in
 * straight away.
 */
export default function GrokAccountModal({ account, onSaved, onClose }: Props) {
  const createAccount = useGrokAccountsStore((s) => s.createAccount)
  const updateAccount = useGrokAccountsStore((s) => s.updateAccount)
  const editing = account !== null

  const [name, setName] = useState(account?.name ?? '')
  const [kind, setKind] = useState<GrokAccountKind>(account?.kind ?? 'device')
  ...
  return (
    <Modal onClose={onClose} data-testid="grok-account-modal">
      <h2>{editing ? `Edit ${account.name}` : 'Add Grok Account'}</h2>
      <form onSubmit={handleSubmit}>
        <div className="form-field">
          <label className="form-label" htmlFor="grok-acct-name">
            Account name
          </label>
          <input
            id="grok-acct-name"
          ...
          <div className="theme-toggle">
            {KIND_OPTIONS.map((k) => (
              <button
                ...
                data-testid={`grok-acct-kind-${k.value}`}
              >
                {k.label}
              </button>
            ))}
        ...
        {kind === 'api_key' ? (
          <div className="form-field">
            <label className="form-label" htmlFor="grok-acct-credential">
              API key
              {editing && ' (leave blank to keep current)'}
            </label>
            <input
              id="grok-acct-credential"
              className="form-input"
              type="password"
              value={credential}
              onChange={(e) => setCredential(e.target.value)}
              placeholder={editing ? 'Keeping current key' : 'xai-…'}
              autoComplete="off"
              data-testid="grok-acct-credential"
            />
          </div>
        ) : (
          !editing && (
            <p className="form-hint" data-testid="grok-acct-device-hint">
              You&apos;ll sign in with Grok in the browser right after adding the account.
            </p>
          )
        )}

        <div className="form-field">
          <label className="form-label" htmlFor="grok-acct-window">
            Usage budget
          </label>
          <select
            id="grok-acct-window"
        ...
        {hasBudget && (
          <>
            <div className="form-row">
              <div className="form-field">
                <label className="form-label" htmlFor="grok-acct-limit-usd">
                  Cost cap (USD)
                </label>
                <input
                  id="grok-acct-limit-usd"
                  className="form-input"
                  type="number"
                  min="0"
                  step="0.01"
                  ...
                  data-testid="grok-acct-limit-usd"
                />
              </div>
              <div className="form-field">
                <label className="form-label" htmlFor="grok-acct-limit-tokens">
                  Token cap
                </label>
                <input
                  id="grok-acct-limit-tokens"
                  className="form-input"
                  type="number"
                  min="0"
                  step="1000"
                  ...
                  placeholder="optional"
                  data-testid="grok-acct-limit-tokens"
                />
              </div>
            </div>
            <div className="form-row">
              <div className="form-field">
                <label className="form-label" htmlFor="grok-acct-warn">
                  Warn at %
                </label>
                <input
                  id="grok-acct-warn"
                  ...
                  data-testid="grok-acct-warn"
                />
              </div>
              <div className="form-field">
                <label className="form-label" htmlFor="grok-acct-critical">
                  Critical at %
                </label>
                <input
                  id="grok-acct-critical"
                  className="form-input"
                  type="number"
                  min="1"
                  max="100"
                  ...
                  data-testid="grok-acct-critical"
                />
              </div>
            </div>
          </>
