...
/** What the import dialog posts: the pasted PEM plus an optional passphrase.
 *  Both are write-only — the server seals them and never returns them. */
...
/** What the generate dialog posts. `key_type` is a fixed option set; the
 *  server only knows how to generate `ed25519` today. */
...
interface SshKeysState {
  ...
  /** The public key text, fetched on demand so the list stays metadata-only
   *  in the UI even though the list response carries it. */
...
/**
 * SSH key vault store. Mutations are admin-only server-side, so every write
 * can fail with a 403 the UI has to show rather than swallow. There is no
 * WebSocket feed for keys — each mutation refetches the list, exactly like
 * the Claude accounts store.
 */
export const useSshKeysStore = create<SshKeysState>((set, get) => ({
  ...
  fetchPublicKey: async (id) => {
    ...
    if (!res.ok) throw new Error(await errorFrom(res, 'Failed to read the public key'))
